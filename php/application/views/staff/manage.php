...
<script type="text/javascript">
    ...
    function search(page) {
        ...
        $.ajax({  
            ...
            url:"<?php echo site_url('staff/stafflist') ?>/"+page,
    ...
    $(document).ready(function(){
        ...
        $('.deactive').live('click',function(){
            var cf = confirm('Confirm request to de-activate staff member?');
            if(!cf)
                return false;
    ...
    <div class="content-header">
        ...
        <?php if (($user[0]->usertype == 'StaffManager')) { ?>
            <div class="content-actions">
                <div class="content-actions">
                    <a href="<?php echo site_url('staff/profile') ?>" class="button">Add Staff</a>
                </div>
            </div>
        <?php } ?>
    </div>
                ...
                <div class="datalist-search">
                    ...
                    <div style="float:left;padding-right:6px">
                        <select id="datalist-filter-type">
                            <option value="all">Show both active and inactive staff</option>
                            <option selected="selected" value="1">Active staff only</option>
                            <option value="0">Inactive staff only</option>
                        </select>
                    </div>
                </div>
