...
class Ion_auth_model extends CI_Model {
    ...
    public function login($identity, $password, $remember = FALSE) {
        ...
        $query = $this->db->select('username, email, id, password, active, last_login,account_number')
        ...
        if ($query->num_rows() == 1) {
            ...
            if ($this->encryption->validate_password($password, $user->password)) {
                if ($user->active == 0) {
                ...
                $session_data = array(
                    ...
                    'account_number' => $user->account_number, //add by donghp
    ...
    public function remove_from_group($group_id = false, $user_id = false) {
        ...
        //if no id was passed use the current users id
        $user_id || $user_id = $this->session->urserdata('user_id');
