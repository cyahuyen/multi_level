    ...
    public function index() {
        ...
        if (($this->input->server('REQUEST_METHOD') === 'POST') && $this->validateForm()) {
            ...
            if (!empty($posts['referring'])) {
                $email_referring = $this->user_model->getEmailbyUser($posts['referring']);
        ...
        $this->data['main_content'] = 'user/register.php';
...
//            $this->form_validation->set_rules('recaptcha_response_field', 'Captcha', 'required|callback_check_captcha');
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $data['main_content'] = 'user/reset_pass_step3.php';
                $this->load->view('home', $data);
            }
        } else if ($step == 3) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $data['main_content'] = 'user/reset_pass_step4.php';
                $this->load->view('home', $data);
            }
        }
        else
    ...
    public function autocomplete() {
        ...
        if ($this->input->post('filter_name')) {
            ...
            $results = $this->user_model->getUsers($data);
